...
void *processPacket(){
	...
	for(;;) {
		...
		//Compare first 4 values of msgage received from buffer with user entered flight computer device serial number
		...
		if(!strncmp(dmsg.text, deviceSerial,4)){
            //parse hexadecimal msgage (packet - this is a char array and somewhat acts like a string) to find the header serial data
            ...
            //parse msgage to find the header tick data and copy this data into the header variable
            copy16bits(temp_uint16, &dmsg, 4);
            sscanf(temp_uint16, "%" SCNx16, &pheader.tick);
            //printf("Device clock value: %" PRId16 "\n", pheader.tick);

            //parse msgage to find the header type data and copy this data into the header variable
...
/*writer thread
This function takes a msgage (newmsg) and passes it to bufferWrite
...
/*
This function writes the msgage passed in the parameter (msgage) to the buffer (buff)
buff is a shared resource between processThread and receiveThread and therefore must be protected
*/
uint8_t bufferWrite(struct msg *msgage, struct buffer *buff) {
	sem_wait(&keySem); //wait on keySem semaphore. This is needed to synchronize access to buff
    	(*buff).msgBuffer[buff->writeIndex] = *msgage; //write msgage to the buffer at index specified by writeIndex
...
/*
This function reads from the buffer (buff) and puts the result in the msgage passed as the parameter (msgage)
buff is a shared resource between processThread and receiveThread and therefore must be protected by a semaphore
*/
void bufferRead(struct msg *msgage, struct buffer *buff) {
	sem_wait(&keySem); //wait on keySem semaphore. This is needed to synchronize access to the buffer.
    	*msgage = (*buff).msgBuffer[buff->readIndex]; //read the buffer value at location specified by readIndex, and put the value read into msgage
...
/*
This function takes a buffer (temp_uint8) and copies 2 bits of the msgage (dmsg), starting at the specified index (i)
This function is needed because the msgage is a string and needs to be parsed before it can be converted to non-char type
*/
...
/*
This function takes a buffer (temp_uint16) and copies 4 bits of the msgage (dmsg), starting at the specified index (i)
This function is needed because the msgage is a string and needs to be parsed before it can be converted to non-char type
*/
...
/*
This function takes a buffer (temp_uint32) and copies 8 bits of the msgage (dmsg), starting at the specified index (i)
This function is needed because the msgage is a string and needs to be parsed before it can be converted to non-char type
*/
